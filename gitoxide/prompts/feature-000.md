...
paragraph and before the crate/feature documentation, stating:
...
Do not modify any `Cargo.toml` `repository`/`homepage` metadata fields across
the workspace. This project is consumed as a git dependency, not published to
crates.io under a new name, so there is no crates.io/docs.rs discoverability
problem to solve by rewriting package metadata — doing so would be
